...
def main(args):
    ...
    if not args.test:
        ...
        logging.info('Create the tokenizer.')
        tokenizer_class = getattr(transformers,config.tokenizer.name)
        tokenizer = tokenizer_class.from_pretrained(config.tokenizer.kwargs.pretrained_model_name_or_path,do_lower_case=config.tokenizer.kwargs.do_lower_case)

        logging.info('Create the training and validation datasets.')
        config.dataset.setdefault('kwargs', {}).update(type_='train',tokenizer=tokenizer)
        train_dataset = _get_instance(src.data.datasets, config.dataset)
        config.dataset.setdefault('kwargs', {}).update(type_='valid',tokenizer=tokenizer)
        ...
        if 'lr_scheduler' in config:
            ...
            # config.lr_scheduler.kwargs.update(num_training_steps=total_steps)
            # lr_scheduler = _get_instance(transformers, config.lr_scheduler, optimizer)
        else:
        ...
        config.logger.setdefault('kwargs', {}).update(log_dir=saved_dir / 'log',
                                                      net=net)
        logger = _get_instance(src.callbacks.loggers, config.logger, tokenizer)
        ...
        config.dataset.setdefault('kwargs', {}).update(type_='test',tokenizer=tokenizer)
